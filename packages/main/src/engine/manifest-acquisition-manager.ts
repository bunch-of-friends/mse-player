...
export class ManifestAcquisitionManager {
    ...
    public async loadStreamDescriptor(): Promise<StreamDescriptor> {
        const manifestAcquisition = await this.sessionStateManager.decorateLoadManifest(() => this.streamTransport.getStreamDescriptor());
